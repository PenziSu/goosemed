...
import { RECIPE_DEEP_LINK_PREFIX } from '../appIdentity';
...
export async function generateDeepLink(recipe: Recipe): Promise<string> {
  ...
  return `${RECIPE_DEEP_LINK_PREFIX}${encoded}`;
}
...
export async function parseDeeplink(deeplink: string): Promise<Recipe | null> {
  try {
    ...
    if (!cleanLink.startsWith(RECIPE_DEEP_LINK_PREFIX)) {
      throw new Error(`Invalid deeplink format. Expected: ${RECIPE_DEEP_LINK_PREFIX}...`);
    }

    const recipeEncoded = cleanLink.replace(RECIPE_DEEP_LINK_PREFIX, '');
