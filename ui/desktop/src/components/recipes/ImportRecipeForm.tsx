...
const i18n = defineMessages({
  ...
  deeplinkPlaceholder: {
    ...
    defaultMessage: 'Paste your goosemed://recipe?config=... deeplink here',
  },
  deeplinkHint: {
    id: 'importRecipeForm.deeplinkHint',
    defaultMessage: 'Paste a recipe deeplink starting with "goosemed://recipe?config="',
  },
  or: {
...
const importRecipeSchema = z
  .object({
    deeplink: z
      .string()
      .refine(
        (value) => !value || value.trim().startsWith('goosemed://recipe?config='),
        'Invalid deeplink format. Expected: goosemed://recipe?config=...'
      ),
    recipeUploadFile: z
      .instanceof(File)
      .nullable()
