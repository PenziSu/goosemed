...
/**
 * Script to unregister ALL goosemed:// protocol handlers
 * Usage: node scripts/unregister-deeplink-protocols.js
 */

const { execSync } = require('child_process');

const PROTOCOL = 'goosemed';

function unregisterAllProtocolHandlers() {
  console.log('Unregistering ALL goosemed:// protocol handlers...');
  
  try {
    // Get all registered GooseMED apps
    console.log('Finding all registered GooseMED applications...');
    ...
    if (pathMatches) {
      pathMatches.forEach(match => {
        ...
        if (path.includes('GooseMED') || path.includes('goosemed')) {
          uniquePaths.add(path);
        }
      });
    }
    
    console.log(`Found ${uniquePaths.size} GooseMED app(s) to unregister:`);
    ...
    const bundleIds = ['io.github.penzisu.goosemed'];
    ...
    console.log(`\n✅ Successfully processed ${unregisteredCount} GooseMED applications`);
    console.log('All goosemed:// protocol handlers have been unregistered.');
    ...
    console.log('1. Use Activity Monitor to quit all GooseMED processes');
    console.log('2. Delete GooseMED apps from Applications folder');
