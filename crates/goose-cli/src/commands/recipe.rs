...
fn generate_deeplink(
    ...
    match recipe_deeplink::encode(&recipe) {
        Ok(encoded) => {
            let mut full_url = format!("goosemed://recipe?config={}", encoded);
...
"#;
    ...
    fn test_handle_deeplink_valid_recipe() {
        ...
        assert!(url.starts_with("goosemed://recipe?config="));
        let encoded_part = url.strip_prefix("goosemed://recipe?config=").unwrap();
